    ...
    <div class="container">
      ...
      <div v-else class="pay-layout">
        <div class="pay-main">
          ...
          <p class="section-subtitle">예약 정보를 확인하고 결제 방법을 선택하세요</p>
          ...
          <!-- Payment Method -->
          <div class="card pay-card">
            <h2 class="pay-section-title">결제 방법</h2>
        ...
        <aside class="pay-sidebar">
          <div class="card" style="position:sticky;top:88px;">
            ...
            <div class="pay-summary-rows">
              <div class="ps-row">
                <span>서비스</span><span>{{ reservation.serviceName }}</span>
              </div>
              <div class="ps-row">
                <span>기본 요금</span><span>{{ reservation.totalPrice?.toLocaleString() }}원</span>
              </div>
...
async function handlePayment() {
  ...
  try {
    ...
    const { orderId, amount, orderName } = prepareRes.data

    // 2. 토스 결제창 호출 (Toss Payments SDK)
    //    SDK가 없을 경우 직접 confirm 호출 (개발 테스트용)
    if (window.TossPayments) {
      const tossPayments = window.TossPayments(import.meta.env.VITE_TOSS_CLIENT_KEY)
      await tossPayments.requestPayment(selectedMethod.value === 'card' ? '카드' : '간편결제', {
        amount,
        orderId,
        orderName,
    ...
    payError.value = e.response?.data?.message || '결제 중 오류가 발생했습니다.'
  } finally {
    paying.value = false
  }
}
...
.pay-avatar { width: 52px; height: 52px; border-radius: 50%; object-fit: cover; }
.pay-name { font-size: 15px; font-weight: 600; margin-bottom: 4px; }
...
.pay-error { color: var(--color-danger); font-size: 13px; text-align: center; margin-top: 10px; }
